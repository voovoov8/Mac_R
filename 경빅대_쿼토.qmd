...
<br> 
- Roe 가 가로축, salary가 세로축인 산포도 indus 별로 다른색
...
step_model <- stepAIC(lm(medv ~ ., data = train_data), 
        direction = "both", 
        trace = FALSE # stepAIC는 MASS 패키지에 포함된 함수로, 단계적 모델 선택을 수행
)

model_selection <- names(coef(step_model))[-1] # coef 가 모델에 있는 회귀 변ㄱ수반환하는 함수 
...
### 3. 정규화 : lasso, ridge (penalized regression)
-> 영향을 줄이는 과정이 필요하다. 
